...
public class HAClusterConfig extends AbstractConfig {
    ...
    private static final String FETCH_MIN_BYTES_DOC = "The minimum amount of data the server should return for " +
            "a fetch request.If insufficient data is available the request will wait for that much data to " +
            "accumulate before answering the request.The default setting of 1 byte means that fetch requests are " +
            "answered as soon as a single byte of data is available or the fetch request times out waiting for data to " +
            "arrive.Setting this to something greater than 1 will cause the server to wait for larger amounts of data " +
    ...
    private static final String FETCH_MAX_BYTES_DOC = "The maximum amount of data the server should return " +
            "for a fetch request." +
            "Records are fetched in batches by the mirror, and if the first record batch in the first non " +
            "empty partition of the fetch is larger than " +
            "this value, the record batch will still be returned to ensure that the mirror can make " +
            "progress.AS such, this is not a absolute maximum." +
            "The maximum record batch size accepted by the broker is defined via " +
            "<code>mes sage.max.bytes</code> (broker config) or " +
    ...
    private static final String FETCH_MAX_WAIT_MS_DOC = "The maximum amount of time the server will block " +
            "before answering the fetch request if there isn't sufficient data to immediately satisfy the requirement " +
            "given by fetch, min, bytes.";

    /* <code>fet ch.backoff.ms</ code>*/
    public static final String FETCH_BACKOFF_MS_CONFIG = "fetch.backoff.ms";
    private static final String FETCH_BACKOFF_MS_DOC = "The amount of time to sleep when fetch partition" +
            "error occurs.";
    /*k <code>metadata.max.age.ms</code> */
    ...
    private static final String MAX_PARTITION_FETCH_BYTES_DOC = "The maximum amount of data per-partition" +
            "the server " +
            "will return.Records are fetched in batches by the mirror.If the first record batch in the" +
            "first non-empty " +
            "partition of the fetch is larger than this limit, the " +
            "batch will still be returned to ensure that the mirror can make progress.The maximum record" +
            "batch size " +
            "accepted by the broker is defined via <code>message.max.bytes</code> (broker config) or " +
            "<code>max.message.bytes</code> (topic config).See " + FETCH_MAX_BYTES_CONFIG + " for limiting" +
            "the mirror request size.";
    ...
    static {
        CONFIG = new ConfigDef()
                ...
                /* default is set to be a bit lower than the server default (10 min), to avoid both client
    and server closing connection at same time */
